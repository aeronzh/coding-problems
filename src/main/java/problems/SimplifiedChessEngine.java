...
private static int[] queen(int[][] board, int player) {
    ...
    private static boolean isQueenSafe(int[][] board, int turn) {
        ...
        if (turn == WHITE && isWinForPlayer(board, WHITE)) {
            return true;
        }

        if (turn == BLACK && isWinForPlayer(board, BLACK)) {
            return true;
        }
    ...
    private static boolean solve(int[][] board, int m, int turn) {
                            ...
                            for (int len = 1; len < 4; len++) {
                                ...
                                if (isValid(board, tr, tc, turn)) {
                                    ...
                                    if (isQueenSafe(board, turn)) {
                                        // Recursive call
                                        if (turn == WHITE) {
                                            if (solve(board, m - 1, turn == WHITE ? BLACK : WHITE)) {
                                                // Move back
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for (int g = 0; g < games; g++) {
            ...
            // Call algorithm
            boolean win = solve(board, m, WHITE);
